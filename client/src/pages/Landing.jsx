...
export default function Landing() {
  ...
  const waNumber   = tenant?.whatsapp      || '233000000000'
  const coverage   = tenant?.coverageArea  || 'Greater Accra'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .land-logo-icon { width: 36px; height: 36px; background: var(--brand); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 16px; flex-shrink: 0; }
        ...
        .land-nav-btn { padding: 9px 22px; background: var(--brand); color: #fff; border: none; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; text-decoration: none; transition: opacity 0.2s; white-space: nowrap; }
        .land-nav-btn:hover { opacity: 0.88; }
        .land-nav-ghost { padding: 9px 18px; background: transparent; color: rgba(240,244,255,0.6); border: 1px solid rgba(255,255,255,0.12); border-radius: 8px; font-size: 13px; font-weight: 500; cursor: pointer; text-decoration: none; transition: all 0.2s; white-space: nowrap; }
        .land-nav-ghost:hover { border-color: var(--brand); color: var(--brand); }
        .land-nav-user { display: flex; align-items: center; gap: 6px; background: rgba(249,115,22,0.1); border: 1px solid rgba(249,115,22,0.2); border-radius: 100px; padding: 6px 14px; font-size: 12px; font-weight: 600; color: var(--brand); text-decoration: none; white-space: nowrap; }
        ...
        .land-hamburger span { display: block; width: 22px; height: 1.5px; background: #fff; }
        ...
        .land-mobile-menu a:hover { color: var(--brand); }
        ...
        .land-hero-badge { display: inline-flex; align-items: center; gap: 8px; background: rgba(249,115,22,0.1); border: 1px solid rgba(249,115,22,0.25); border-radius: 100px; padding: 6px 16px; margin-bottom: 28px; font-size: 12px; font-weight: 500; color: var(--brand); }
        .land-hero-badge-dot { width: 6px; height: 6px; background: var(--brand); border-radius: 50%; animation: pulse 2s infinite; }
        ...
        .land-hero-title span { color: var(--brand); }
        ...
        .land-btn-primary { padding: 14px 32px; background: var(--brand); color: #fff; border: none; border-radius: 10px; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }
        ...
        .land-track-input:focus { border-color: var(--brand); }
        .land-track-btn { padding: 12px 22px; background: var(--brand); color: #fff; border: none; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; white-space: nowrap; }
        ...
        .land-section-tag { font-size: 11px; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; color: var(--brand); margin-bottom: 14px; }
        ...
        .land-step { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 16px; padding: 26px; transition: border-color 0.2s; }
        .land-step:hover { border-color: rgba(249,115,22,0.25); }
        .land-step-num { width: 40px; height: 40px; background: rgba(249,115,22,0.12); border: 1px solid rgba(249,115,22,0.25); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-family: 'Syne', sans-serif; font-weight: 800; font-size: 15px; color: var(--brand); margin-bottom: 16px; }
        ...
        .land-price-badge { position: absolute; top: 12px; right: 12px; background: var(--brand); color: #fff; font-size: 10px; font-weight: 700; padding: 3px 8px; border-radius: 100px; }
        ...
        .land-price-fee { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 24px; color: var(--brand); margin-bottom: 4px; }
        ...
        .land-footer-logo-icon { width: 28px; height: 28px; background: var(--brand); border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 12px; }
        ...
        .land-footer-links a:hover { color: var(--brand); }
      ...
      <div className="land-root">
        ...
        <div className={`land-mobile-menu${menuOpen ? ' open' : ''}`}>
          ...
          {customer
            ? <a href="/account" onClick={() => setMenuOpen(false)} style={{ color: brandColor }}>My Account →</a>
            : <a href="/login" onClick={() => setMenuOpen(false)}>Sign In</a>
          }
          <a href="/book" onClick={() => setMenuOpen(false)} style={{ color: brandColor }}>Book Delivery →</a>
        </div>
        ...
        <nav className="land-nav">
          ...
          <div className="land-nav-right">
            <a href="/track" className="land-nav-ghost">🔍 Track</a>
            {customer ? (
              <a href="/account" className="land-nav-user">👤 {customer.name?.split(' ')[0]}</a>
            ) : (
              <a href="/login" className="land-nav-ghost">Sign In</a>
            )}
        ...
        <section className="land-hero">
          <div className="land-hero-badge">
            ...
            {coverage}'s Trusted Delivery Service
          </div>
          ...
          <p className="land-hero-sub">
            ...
            {bizName} moves your packages across {coverage} — fast, safe, and reliably.
          </p>
          ...
          <div className="land-track-bar" id="track">
            ...
            <form className="land-track-form" onSubmit={handleTrack}>
              <input className="land-track-input" placeholder="Enter your Order ID e.g. SWG-X7K2M9" value={trackID} onChange={e => setTrackID(e.target.value)} />
          ...
          <div className="land-section-inner">
            ...
            <div className="land-steps">
              {[
                ...
                { num:'02', title:'Get Your Order ID', text:'Receive a unique Order ID instantly. Share it with your recipient so they can track too.' },
                { num:'03', title:'Rider Assigned', text:'Our riders see your order and the first available one accepts it immediately.' },
        ...
        <section className="land-section" id="pricing">
          <div className="land-section-inner">
            ...
            <div className="land-pricing">
              {[
                { icon:'🚶', name:'Standard',        desc:'Reliable delivery within the day. Perfect for non-urgent packages.',          fee:`GHS ${tenant?.standardFee || 30}`,  eta:'2–4 hours',       featured:false },
                { icon:'⚡', name:'Same-Day',         desc:'Book before noon and your package arrives same day.',                         fee:`GHS ${tenant?.sameDayFee || 50}`,   eta:'1–3 hours',       featured:false },
                { icon:'🚀', name:'Express / Urgent', desc:'Priority delivery for time-sensitive packages. Our fastest service.',         fee:`GHS ${tenant?.expressFee || 80}`,   eta:'30–60 mins',      featured:true  },
                { icon:'📅', name:'Scheduled',        desc:'Pick your preferred date and time. We plan around your schedule.',            fee:`GHS ${tenant?.scheduledFee || 40}`, eta:'Your chosen time', featured:false },
              ].map(t => (
            ...
            <div style={{ textAlign:'center', marginTop:28, fontSize:13, color:'rgba(240,244,255,0.3)' }}>
              All fees are base rates within {coverage}. Final price includes distance-based calculation.
            </div>
          </div>
        </section>
        ...
        <section className="land-section" id="coverage" style={{ background:'rgba(255,255,255,0.02)' }}>
          <div className="land-section-inner">
            ...
            <div className="land-coverage">
              ...
              <div>
                <div className="land-coverage-title">{coverage}</div>
                <p className="land-coverage-text">
                  {bizName} currently operates across {coverage} and surrounding areas — fast, safe and reliable delivery for your packages.
                </p>
          ...
          <div className="land-section-inner">
            <div className="land-cta">
              ...
              <p className="land-cta-sub">
                {customer
                  ? `Welcome back, ${customer.name?.split(' ')[0]}! Book a delivery or check your order history.`
                  : `Book online in minutes or reach us directly on WhatsApp. ${bizName} will take it from there.`
                }
              </p>
              <div className="land-cta-actions">
                ...
                {customer ? (
                  <a href="/account" className="land-btn-secondary" style={{ fontSize:15, padding:'15px 36px' }}>📋 My Orders</a>
                ) : (
                  <a href={`https://wa.me/${waNumber}?text=${encodeURIComponent(`Hi ${bizName}! I'd like to book a delivery.`)}`} target="_blank" rel="noreferrer" className="land-cta-wa">
        ...
        <footer className="land-footer">
          ...
          <div className="land-footer-copy">© {new Date().getFullYear()} {bizName} Delivery Service. All rights reserved.</div>
        </footer>

        {/* WHATSAPP FLOAT */}
        <a href={`https://wa.me/${waNumber}?text=${encodeURIComponent(`Hi ${bizName}! I'd like to book a delivery.`)}`} target="_blank" rel="noreferrer" className="land-wa">
